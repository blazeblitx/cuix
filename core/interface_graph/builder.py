"""
CUIX Phase 2 & Task 3: Python Interface Graph Builder & Multi-Signal Confidence Parser
Parses, queries, and analyzes site-agnostic UI graphs with confidence scores.
"""
...
class InterfaceGraphAnalyzer:
    ...
    def find_nodes_by_role(self, target_role: str, min_confidence: float = 0.0) -> List[Dict[str, Any]]:
        """Recursively retrieves all UI elements matching a specific role above a min confidence threshold."""
        results: List[Dict[str, Any]] = []

        def traverse(node: Dict[str, Any]):
            role = node.get('role')
            confidence = node.get('confidence', 1.0)
            if role == target_role and confidence >= min_confidence:
                results.append(node)
    ...
    def get_actionable_elements(self, min_confidence: float = 0.60) -> List[Dict[str, Any]]:
        """Returns all buttons, links, search inputs, and filters with confidence >= threshold."""
        ...
        def traverse(node: Dict[str, Any]):
            role = node.get('role')
            confidence = node.get('confidence', 1.0)
            if role in actionable_roles and confidence >= min_confidence:
                results.append(node)
            for child in node.get('children', []):
                traverse(child)
    ...
    def compute_interface_complexity_score(self) -> float:
        """Calculates visual complexity load score from 0.0 (Minimalist) to 1.0 (Cluttered)."""
    ...
    def get_average_confidence(self) -> float:
        """Returns average classification confidence across all graph nodes."""
        return float(self.summary.get('avgConfidence', 0.85))
